...
export const ServerChannel = ({channel,server,role}:ServerChannelProps) => {
  ...
  return (
    <button onClick={onClick} className={cn("group px-2 py-2 rounded-md flex items-center gap-x-2 w-full hover:bg-slate-700/10 transition mb-1",params?.channelId===channel.id && "bg-slate-700")}>
      {icon}
      ...
      {channel.name !== "general" && role !== MemberRole.GUEST && (
        <div className="ml-auto flex items-center gap-x-2">
            <ActionTooltip label="Edit">
                <Edit  onClick={(e) => onAction(e, "editChannel")} className="hidden group-hover:block w-4 h-4 text-slate-400 hover:text-slate-300 transition"/>
            </ActionTooltip>
            <ActionTooltip label="Delete">
                <Trash  onClick={(e) => onAction(e, "deleteChannel")} className="hidden group-hover:block w-4 h-4 text-slate-400 hover:text-slate-300 transition"/>
            </ActionTooltip>
        </div>
      )}
